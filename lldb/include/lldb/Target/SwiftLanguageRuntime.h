...
protected:
  ...
  ///   unfortunatley process wants to cache the returned language
  ///   runtimes and doesn't call CreateInstance ever again.
  std::unique_ptr<SwiftLanguageRuntime> m_stub;
  std::unique_ptr<SwiftLanguageRuntime> m_impl;
...
public:
  ...
  GetMemberVariableOffset(CompilerType instance_type, ValueObject *instance,
                          ConstString member_name, Status *error = nullptr);

  /// Ask Remote Mirrors for the size of a Swift type.
  virtual llvm::Optional<uint64_t> GetBitSize(CompilerType type);

  /// Ask Remote mirrors for the stride of a Swift type.
  virtual llvm::Optional<uint64_t> GetByteStride(CompilerType type);

  /// Ask Remote mirrors for the alignment of a Swift type.
  virtual llvm::Optional<size_t> GetBitAlignment(CompilerType type);
  ...
  virtual void ReleaseAssociatedRemoteASTContext(swift::ASTContext *ctx);

  virtual void AddToLibraryNegativeCache(llvm::StringRef library_name);
  virtual bool IsInLibraryNegativeCache(llvm::StringRef library_name);
  ...
  virtual lldb::SyntheticChildrenSP
  GetBridgedSyntheticChildProvider(ValueObject &valobj);

  /// Expression Callbacks.
  /// \{
  virtual void WillStartExecutingUserExpression(bool);
  virtual void DidFinishExecutingUserExpression(bool);
  /// \}

  virtual bool IsABIStable();
